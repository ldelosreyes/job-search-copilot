...
function renderCard(queryClient: QueryClient, overrides: Partial<Application> = {}) {
  return render(
...
describe("ApplicationCard fit-result loading state", () => {
  ...
  test("shows a loading skeleton for every scoreable card while a bulk score-all run is in flight", async () => {
    const queryClient = new QueryClient();
    renderCard(queryClient);
  ...
  test("does not show a loading skeleton during a bulk run for a card with no job description", () => {
    const queryClient = new QueryClient();
    renderCard(queryClient, { jdText: null, fitScore: null, fitRationale: null });
